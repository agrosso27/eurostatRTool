...
A standout feature of eurostatRToolCustom is the storyboard layout from `Flexdashboard` that organises information into a sequence of slides. 
...
Specifically, the eurostatRToolCustom package can be installed from GitHub by
running:

``` r
devtools::install_github("eurostat/eurostatRToolCustom")
```
...
mandatory. For more information, you should consult the [Wiki pages for this project](https://github.com/eurostat/eurostatRToolCustom/wiki). 

## Getting started

The eurostatRToolCustom package requires some learning to be
...
eurostatRToolCustom::generate_dashboard("path/to/output/folder")
```

To override the primary menu categories for a single run, pass them directly:

``` r
eurostatRToolCustom::generate_dashboard(
...
For general help with the eurostatRToolCustom package, you should consult the [Wiki pages for this project](https://github.com/eurostat/eurostatRToolCustom/wiki) 
or reffer to the package’s documentation. After instalation you can access detailed information on individual functions by using `?function_name` and explore comrehensive guides through the vignettes by running `utils::browseVignettes("eurostatRToolCustom")`.
