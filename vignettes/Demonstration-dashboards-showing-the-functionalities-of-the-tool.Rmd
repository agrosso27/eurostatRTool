...
* `update_data("library_path/eurostatRToolCustom/extdata/data-italy.csv")`
* `update_forecasts("library_path/eurostatRToolCustom/extdata/forecasts-italy.xlsx")`
* `update_dim_labels("library_path/eurostatRToolCustom/extdata/dim-labels-italy.csv")`
* `update_dashboard_structure("library_path/eurostatRToolCustom/extdata\\dashboard-structure-italy.xlsx")`
...
While all other options -see the vignette `General-workflow-for-building-and-maintaining-your-dashboard` for a full description of the available functionalities- are customisable: colours, titles, headers, footer, logo, data, indicators, visualisation modes, etc., the topics structuring the main menu can also be adjusted. Use `update_menu_categories()` to save your preferred categories or pass them on the fly via the `menu_categories_override` argument of `generate_dashboard()`.
