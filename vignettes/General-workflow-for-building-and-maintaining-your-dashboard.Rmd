...
Important: The templates of the files mentioned below you can find in the `library_path/eurostatRToolCustom/extdata/` folder,
where `library_path` is your folder with R packages, which you can find using `.libPaths()`. You can copy those files into your folder.
...
The sample files are available from the [GitHub page of the project](https://github.com/Randbee/eurostatRToolCustom/), in the folder `inst` and within the later, in the `extdata` folder. 
...
`dashboard-structure.xlsx` and `dashboard-structure-italy.xlsx`, for the Eurostatistics and NSI scenarios, respectively. The both scenarios (Eurostatistics and NSI scenarios) have different topics in their respective main menus.
...
While all other options are customisable: colours, titles, headers, footer, logo, data, indicators, visualisation modes, etc., you can also tailor the main menu categories. Persist them with `update_menu_categories()` or provide them when calling `generate_dashboard(menu_categories_override = c("Topic 1", "Topic 2"))`.
...
In principle, this double scale plot can be used to visualise any data series with different scale and not only decomposition results. However, as currently there is no specific function for this aim in the eurostatRToolCustom package, in case the dashboard manager wants to change the labels in the filtering options he/she will have to modify this text directly in the source code.
