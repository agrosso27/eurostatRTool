...
output: 
  flexdashboard::flex_dashboard:
    css: !expr eurostatRToolCustom::css()
    orientation: rows
    vertical_layout: fill
    logo: !expr eurostatRToolCustom::logo()
    includes:
      in_header: !expr eurostatRToolCustom::social()
      after_body: !expr eurostatRToolCustom::footer()
...
menu_categories_vec <- params$menu_categories
```
...
menu_cat <- intersect(unique(dashboard_structure_data$menu_position), 1:length(menu_categories_vec))
...
for (i in menu_cat) {
  ...
  assign("menu_category", menu_categories_vec[i], menu_subpage_env)
